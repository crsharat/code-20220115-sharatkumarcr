...
from code.exceptions import InvalidPatientRecord
...
def count_total_number_of_overweighted_patients(json_records: list):
    ...
    return type: int
    """
    return len(list(filter(lambda patient_record: is_overweight(patient_record['bmi']),
...
def get_bmi_of_all_patients(json_records: list):
    ...
    try:
        ...
        raise InvalidPatientRecord("Patient record is not valid")
